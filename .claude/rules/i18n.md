---
paths:
  - "crates/roadie-ui/locales/**"
  - "crates/roadie-ui/src/catalog_parity.rs"
  - "crates/roadie-core/src/locale.rs"
  - "crates/roadie-desktop/src/services/i18n.rs"
  - ".config/crowdin.yml"
  - ".github/scripts/i18n/**"
...
- The locale key test (`roadie-ui/src/catalog_parity.rs`,
...
- Adding a locale: drop the `.yml` in `crates/roadie-ui/locales/` and add the
  `SUPPORTED` entry in `roadie-core`'s `locale.rs` (picker order: native-name
...
- Check with `cargo test -p roadie-ui locale` (catalog parity, runs everywhere)
  and `cargo test -p roadie-desktop i18n` (end-to-end key resolution; CI's Linux
