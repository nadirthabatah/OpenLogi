#!/usr/bin/env bash
# Read-only macOS permission diagnosis for OpenRoadie. Safe to hand to a reporter:
# it changes nothing. One step needs root; without it the script prints the
# command to run instead.
set -uo pipefail

app=${ROADIE_APP:-/Applications/OpenRoadie.app}
# The helper is named after its display name; bundles built before that rename
# spell it without the space. Same identity (org.roadie.agent) either way.
helper_bundle() {
  for name in "OpenRoadie Agent.app" "OpenRoadieAgent.app"; do
    [ -d "$1/Contents/Library/LoginItems/$name" ] && {
      printf '%s\n' "$1/Contents/Library/LoginItems/$name"
      return
    }
  done
  printf '%s\n' "$1/Contents/Library/LoginItems/OpenRoadie Agent.app"
}
...
pid=$(pgrep -x roadie-agent | head -1)
if [ -z "${pid:-}" ]; then
  echo "   no roadie-agent process — the GUI cannot show devices without it"
else
  count=$(pgrep -x roadie-agent | wc -l | tr -d ' ')
  if [ "$count" -gt 1 ]; then
    echo "   WARNING: $count agent processes are running — each is its own TCC identity:"
    pgrep -x roadie-agent | while read -r p; do
...
for target in "$app" "$agent" "$app/Contents/MacOS/roadie"; do
  [ -e "$target" ] || {
    echo "   missing: $target"
    continue
  }
...
cat <<TXT
   ...
   predates the failure, reproduce it in the foreground:
     ...
     ROADIE_LOG=debug "$agent/Contents/MacOS/roadie-agent"
