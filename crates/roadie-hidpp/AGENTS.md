# roadie-hidpp — HID++ protocol, hard fork
...
- `[lib] name = "hidpp"` in `Cargo.toml`. Every consumer imports it as
  ...
  `crates/roadie-hid/src/**`, plus a doctest in this crate's own `src/lib.rs`.
...
- Feature `0x0005` (`device_type_and_name`) is one of four incompatible "device
  ...
  mixing them by raw value lives in `crates/roadie-device/AGENTS.md` (the
  `roadie-hid` side), not duplicated here.
...
converted to `roadie-core`'s own structs first. Dropping it would be a large
...
OpenRoadie-internal?) rather than by drive-by.
