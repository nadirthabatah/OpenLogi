...
//!
//! Quit OpenRoadie first: one HID node cannot serve two channels, and the agent
//! holds one whenever it captures.
//!
//! ```sh
//! cargo run -p roadie-hid --example thumbwheel_trace -- <receiver-uid> <slot>
//! # e.g. the receiver id and slot printed by `roadie list`
//! ```

use std::sync::Arc;
use std::time::Duration;

use roadie_hid::thumbwheel::{self, Thumbwheel, WheelDirection};
use roadie_hid::{ChannelPool, DeviceRoute, host};
