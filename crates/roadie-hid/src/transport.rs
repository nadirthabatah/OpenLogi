...
//! enumeration time (see [`roadie_catalog::hidpp::LONG_COLLECTIONS`]) and reporting support
...
use roadie_device::backend::{BackendError, HotplugEvent, NodeId, NodeInfo};
use roadie_device::write::matches_litra;
...
/// now that the contract lives in `roadie-device`, which is the orphan rule
        ...
        BackendError::Backend(message) => {
            let hint = if crate::permissions::has_access() {
                ...
                "Input Monitoring is NOT granted to this process; grant it to \
                 OpenRoadie Agent under System Settings → Privacy & Security → \
                 Input Monitoring"
            };
...
///
/// The table itself lives in [`roadie_catalog::hidpp`], because which
...
fn is_hidpp_long_collection(usage_page: u16, usage_id: u16) -> bool {
    roadie_catalog::hidpp::is_long_collection(usage_page, usage_id)
}
...
/// pages not in [`roadie_catalog::hidpp::LONG_COLLECTIONS`].
...
fn is_long_only_collection(usage_page: u16, usage_id: u16) -> bool {
    roadie_catalog::hidpp::is_long_only(usage_page, usage_id)
}
...
pub(crate) async fn enumerate_devices() -> Result<Vec<async_hid::Device>, BackendError> {
    ...
    // can be diagnosed from `ROADIE_LOG=debug` without a rebuild.
