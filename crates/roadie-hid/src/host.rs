...
use roadie_core::device::{DeviceInventory, StandaloneDevice};
use roadie_core::hid::{LightCommand, PairingError, WriteError};

use crate::probe_cache::FileProbeCacheStore;
use crate::transport::native_backend;
use roadie_core::hid::smartshift::{SmartShiftAutoDisengage, SmartShiftMode, SmartShiftStatus};
use roadie_device::ChannelPool;
use roadie_device::DeviceRoute;
use roadie_device::backend::{HidBackend, HotplugStream};
use roadie_device::backlight::BacklightState;
use roadie_device::inventory::{Enumerator, InventoryError};
use roadie_device::pairing::PairingReceiver;
use roadie_device::write::{
...
pub async fn enumerate_standalone() -> Result<Vec<StandaloneDevice>, InventoryError> {
    roadie_device::inventory::standalone::enumerate_standalone(&*native_backend()).await
}

/// Subscribe to this host's HID hotplug events.
pub fn watch_hotplug() -> Result<HotplugStream, InventoryError> {
    roadie_device::inventory::hotplug::watch_hotplug(&*native_backend())
}
...
pub async fn enumerate() -> Result<Vec<DeviceInventory>, InventoryError> {
    roadie_device::inventory::enumerate(native_backend()).await
}
...
pub async fn list_pairing_receivers() -> Result<Vec<PairingReceiver>, PairingError> {
    roadie_device::pairing::list_pairing_receivers(&*native_backend()).await
}
