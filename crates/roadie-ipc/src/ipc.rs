...
pub const RUN_ENV: &str = "ROADIE_RUN";
...
pub struct ForegroundApps {
    ...
    /// usable backend). OpenRoadie's own processes are reported here like any
    ...
    /// identifier and capped at [`RECENT_APPS`]. Excludes OpenRoadie's own
...
/// full `roadie_hid::DiscoveredDevice` (kind, auth bits) internally, keyed by
...
/// Mirrors `roadie_hid::PairingEvent` but in a wire-safe form — the discovered
...
/// button/scroll/interrupt view of what OpenRoadie's hook actually receives.
///
/// bincode encodes the variant *index*, so variants are append-only.
...
pub enum ClientKind {
    ...
    Gui,
    /// The `roadie` CLI reading a snapshot; served without arming.
    Cli,
