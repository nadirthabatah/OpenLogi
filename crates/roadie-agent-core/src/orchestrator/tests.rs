...
use roadie_core::binding::{Action, Binding, ButtonId};
...
use roadie_hid::{DIRECT_DEVICE_INDEX, DeviceRoute};
use std::sync::Arc;
...
fn dev(key: &str, slot: u8, online: bool) -> AgentDevice {
    AgentDevice {
        ...
        unit_id: [0; 4],
        capabilities: None,
        kind: roadie_core::device::DeviceKind::Mouse,
...
fn raw_light_dev(key: &str) -> AgentDevice {
    AgentDevice {
        ...
        light_capabilities: Some(roadie_core::device::LightCapabilities {
            power: true,
            ..roadie_core::device::LightCapabilities::default()
        }),
        online: true,
    }
}
...
fn build_devices_keeps_serial_backed_standalone_lights_beside_hidpp_devices() {
    let light_capabilities = roadie_core::device::LightCapabilities {
        power: true,
        ..roadie_core::device::LightCapabilities::default()
    };
