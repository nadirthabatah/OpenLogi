...
//! note in `roadie-hook`); it locks and pushes only once the GUI starts
...
mod tests {
    use super::*;
    use roadie_core::binding::ButtonId;
    ...
    fn bounded_buffer_drops_oldest() {
        ...
        for _ in 0..(CAPACITY + 10) {
            m.record(&MouseEvent::Scroll {
                delta: roadie_hook::ScrollDelta::wheel_ticks(0.0, 1.0),
                from_trackpad: false,
                device: None,
            });
        }
