...
use roadie_core::binding::{Action, Binding};
use roadie_core::bindings::{button_bindings_for, oshook_gestures_for};
use roadie_core::config::{Config, LightSettings, ScrollResolution};
use roadie_core::device::{
    Capabilities, DeviceInventory, DeviceKind, LightCapabilities, StandaloneDevice,
};
use roadie_core::device_order::{DeviceIdentity, DeviceStableId};
use roadie_hid::{
...
/// fallback agrees with the GUI carousel — see [`roadie_core::device_order`]).
struct AgentDevice {
    config_key: String,
    model_key: String,
...
impl Orchestrator {
    ...
    fn hook_maps_for(&self, key: Option<&str>, app: Option<&str>) -> HookMaps {
        ...
        // the user asked OpenRoadie to leave alone.
    ...
    fn reapply_volatile_settings(&self, dev: &AgentDevice) {
        ...
        let smartshift = device
            ...
            .map(roadie_hid::SmartShiftStatus::from);
