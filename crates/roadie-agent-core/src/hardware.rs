...
/// counterpart of `roadie_hid::write::with_route`'s "boilerplate-eater"
...
pub fn toggle_smartshift_in_background(
    ...
    DeviceOp::new(capture, registry, receiver_access, &target).spawn_write(
        "SmartShift toggle",
        |c| async move { roadie_hid::toggle_smartshift_on(&c).await },
...
/// via [`roadie_hid::set_fn_lock_on`]. Returns immediately; failures (incl.
...
pub fn write_fn_lock_in_background(op: DeviceOp<'_>, on: bool) {
    ...
    op.spawn_write(
        "Fn-lock write",
        move |c| async move { roadie_hid::set_fn_lock_on(&c, on).await },
        move |result| match result {
...
pub fn reapply_mouse_volatile_in_background(
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            if let Some(dpi) = dpi {
                let result = tokio::time::timeout(WRITE_BUDGET, async {
                    roadie_hid::set_dpi_on(&shared, dpi).await
                })
                .await;
                match result {
                    Ok(Ok(())) => {
            ...
            if let Some(ss) = smartshift {
                let result = tokio::time::timeout(WRITE_BUDGET, async {
                    roadie_hid::set_smartshift_on(&shared, ss).await
                })
                .await;
                match result {
                    Ok(Ok(())) => debug!(
...
async fn apply_wheel_mode(
    ...
    match (resolution, inverted) {
        (Some(resolution), Some(inverted)) => {
            roadie_hid::set_scroll_wheel_mode_on(shared, resolution, inverted)
                .await
                .map(|_| ())
        }
        (Some(resolution), None) => roadie_hid::set_scroll_resolution_on(shared, resolution)
            .await
            .map(|_| ()),
        (None, Some(inverted)) => roadie_hid::set_scroll_inversion_on(shared, inverted).await,
        (None, None) => Ok(()),
    }
}
...
pub fn write_dpi_in_background(
    ...
    DeviceOp::new(capture, registry, receiver_access, &target).spawn_write(
        "DPI write",
        move |c| async move { roadie_hid::set_dpi_on(&c, dpi).await },
        move |result| match result {
        ...
        move |shared| async move {
            match change {
                ...
                ScrollWheelModeChange::Resolution(resolution) => {
                    roadie_hid::set_scroll_resolution_on(&shared, resolution)
                        .await
                        .map(|_| ())
                }
                ScrollWheelModeChange::Inversion(inverted) => {
                    roadie_hid::set_scroll_inversion_on(&shared, inverted).await
                }
            }
        },
...
/// [`roadie_hid::set_keyboard_color_on`]. A registry miss and write
/// failures are logged, not surfaced.
pub fn set_lighting_in_background(op: DeviceOp<'_>, lighting: &Lighting) {
    let (r, g, b) = lighting_rgb(lighting);
    op.spawn_write(
        "lighting write",
        move |c| async move { roadie_hid::set_keyboard_color_on(&c, r, g, b).await },
        move |result| match result {
