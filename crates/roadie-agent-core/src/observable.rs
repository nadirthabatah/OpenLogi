...
use roadie_core::brand::is_roadie_foreground_id;
...
impl ObservableState {
        ...
        let (tx, _) = watch::channel(Observation {
            ...
            snapshot: AgentSnapshot {
                status: AgentStatus {
                    ...
                    agent_version,
                    input_monitoring_granted: roadie_hid::permissions::has_access(),
    ...
    /// Serve one [`Agent::observe`](roadie_ipc::Agent::observe): wait until
    ...
    ///
    /// `current` mirrors the matcher exactly, OpenRoadie's own processes
    /// included; the recent list filters them out, because a per-app profile
    /// for OpenRoadie is never what a user means. The list grows only here, so a
    ...
    pub fn set_foreground(&self, app: Option<ForegroundApp>) {
        self.update(|snapshot| {
            ...
            if let Some(app) = &app
                && !is_roadie_foreground_id(&app.id)
            {
...
mod tests {
    ...
    use roadie_core::device::{DeviceInventory, DeviceKind, PairedDevice, ReceiverInfo};
    use roadie_hid::DIRECT_DEVICE_INDEX;
    use roadie_ipc::{InventoryHealth, RECENT_APPS};
    use std::sync::Arc;
    ...
    fn our_own_windows_never_become_a_profile_target() {
        let state = state();
        front(&state, "com.apple.Safari");
        // The user clicked over to OpenRoadie to edit Safari's profile: the
