...
use roadie_hid::{CaptureChannel, ChannelRegistry};
...
struct HeldShortcuts {
    by_press: HashMap<PressToken, roadie_inject::HeldChord>,
}
    ...
    fn start(&mut self, press: &PressToken, action: &Action) -> bool {
        ...
        match self.by_press.entry(press.clone()) {
            ...
            std::collections::hash_map::Entry::Vacant(slot) => {
                slot.insert(roadie_inject::press_hold(combo));
            }
        }
        true
    }
...
impl ActionDispatcher {
    ...
    /// [`roadie_hook::MouseEvent::CaptureInterrupted`].
