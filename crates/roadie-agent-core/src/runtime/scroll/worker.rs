...
mod tests {
    ...
    fn cancellation_overtakes_a_full_queue_without_discarding_another_source() {
        ...
        loop {
            ...
            output.push(frame);
            if frame.phase == roadie_inject::SmoothScrollPhase::Ended {
                break;
            }
        }
        ...
        assert!(
            output
                .iter()
                .all(|frame| frame.phase != roadie_inject::SmoothScrollPhase::Cancelled)
        );
    }
    ...
    fn stale_overflow_control_does_not_cancel_current_generation_motion() {
        ...
        assert_eq!(
            frames
                ...
                .phase,
            roadie_inject::SmoothScrollPhase::Began
        );
        ...
        let terminal = loop {
            ...
            if matches!(
                phase,
                roadie_inject::SmoothScrollPhase::Ended
                    | roadie_inject::SmoothScrollPhase::Cancelled
            ) {
                break phase;
            }
        };
    ...
    fn generation_invalidation_cancels_started_output_out_of_band() {
        ...
        loop {
            let phase = received
                ...
                .phase;
            if phase == roadie_inject::SmoothScrollPhase::Cancelled {
                break;
            }
            assert_eq!(phase, roadie_inject::SmoothScrollPhase::Changed);
        }
        runtime.shutdown();
    }
}
