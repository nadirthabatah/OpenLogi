...
use super::*;
use roadie_core::binding::{GESTURE_SWIPE_THRESHOLD, LongPressBinding};
...
// `roadie-core`; these cover only what `HoldState` adds on top — tagging a
...
fn scroll_interception_uses_the_button_source_safety_policy_and_skips_trackpads() {
    let logitech = EventDevice {
        vendor_id: Some(roadie_hook::LOGITECH_VENDOR_ID),
