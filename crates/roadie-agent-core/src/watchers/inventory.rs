...
//! `roadie_hid::enumerate` every `period` and forwards each completed
//! snapshot over an unbounded mpsc to the agent's select loop, which applies
//! it via `Orchestrator::refresh_inventory`.
//!
//! An OS hotplug event (`roadie_hid::watch_hotplug`) cuts the wait short so
...
use roadie_hid::ChannelRegistry;
use tokio::sync::mpsc;
...
impl WatchState {
    ...
    fn classify_parts(
        &mut self,
        inventories: Vec<DeviceInventory>,
        standalone: Result<Vec<StandaloneDevice>, roadie_hid::InventoryError>,
        hid_open_failures: bool,
    ) -> InventoryEvent {
        self.succeeded = true;
    ...
    ///   `Ok`: `roadie_hid`'s `NodeLedger` replays the node's last inventory
    ///   (#218/#222).
    ...
    fn classify(
        &mut self,
        result: Result<(Vec<DeviceInventory>, Vec<StandaloneDevice>), roadie_hid::InventoryError>,
...
fn spawn_inner(
    ...
    let spawn_result = thread::Builder::new()
        .name("roadie-inventory-watcher".into())
        .spawn(move || {
            ...
            let mut enumerator = roadie_hid::host::persisted_enumerator();
            ...
            let mut hotplug = match rt.block_on(async { roadie_hid::watch_hotplug() }) {
            ...
            loop {
                ...
                let event = match rt.block_on(enumerator.enumerate()) {
                    Ok(inventories) => {
                        let standalone = rt.block_on(roadie_hid::enumerate_standalone());
...
mod tests {
    ...
    use roadie_hid::{BackendError, InventoryError};
