...
//!
//! Runs [`roadie_hid::run_capture_session`] concurrently for every device in
...
//!   [`ThumbwheelScrollUp`](roadie_core::binding::ButtonId::ThumbwheelScrollUp) /
//!   [`ThumbwheelScrollDown`](roadie_core::binding::ButtonId::ThumbwheelScrollDown)
...
use roadie_core::config::ThumbwheelSensitivity;
use roadie_core::scroll::ScrollDelta;
use roadie_hid::session::gesture::{CaptureSpec, GESTURE_SOURCE_BUTTONS};
use roadie_hid::{CaptureChannel, CapturedInput, DeviceRoute, run_capture_session};
...
impl GestureOutputs {
    ...
    fn post_scroll(&self, session: &HidppSessionId, delta: ScrollDelta) {
        if !self.scroll.try_hidpp_scroll(session, delta) {
            ...
            roadie_inject::post_scroll(delta);
        }
    }
}
...
fn spawn_session(
    ...
    tokio::spawn(async move {
        let _lease = lease;
        let backend = roadie_hid::host::backend();
