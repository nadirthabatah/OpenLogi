...
//! through `roadie://` deeplinks there, and Windows has no scheme
//! registration yet — so just "Show Main Window" (also the left-click action)
//! and "Quit OpenRoadie". Show focuses the running GUI if there is one (a
//! second launch would exit on the `roadie.lock` singleton) or spawns the
//! sibling `OpenRoadie.exe` / `roadie-desktop.exe`. Quit terminates the GUI
...
/// agent's lifetime.
fn run_tray_loop() {
    let class_name = wide("OpenRoadieAgentTray");
...
unsafe fn show_menu(hwnd: HWND) {
    ...
    unsafe {
        ...
        AppendMenuW(
            ...
            MF_STRING,
            ID_QUIT,
            wide(&rust_i18n::t!("Quit OpenRoadie")).as_ptr(),
        );
...
/// GUI is running (a second launch would just exit on the `roadie.lock`
...
/// PIDs of this user's running GUI processes: `OpenRoadie.exe` (installed /
/// portable layout) or `roadie-desktop.exe` (cargo target dir).
///
/// Matching by *name* rather than by install directory is deliberate: the
/// GUI is a per-user singleton (`roadie.lock` lives under the profile), so
...
///
/// `OpenRoadie.exe` is matched case-*sensitively*: the CLI is `roadie.exe`,
...
fn is_gui_process_name(name: &str) -> bool {
    name == "OpenRoadie.exe" || name.eq_ignore_ascii_case("roadie-desktop.exe")
}
...
fn spawn_gui() {
    ...
    // Dev target dir first: it holds both `roadie.exe` (CLI) and
    // `roadie-desktop.exe`, and the CLI shares `OpenRoadie.exe`'s name on the
    // case-insensitive filesystem — so `dir.join("OpenRoadie.exe").exists()`
    // there resolves to the CLI and would launch it. Probing the unambiguous
    // `roadie-desktop.exe` first avoids that; the installed layout has only
    // `OpenRoadie.exe` and falls through to it.
    let gui = ["roadie-desktop.exe", "OpenRoadie.exe"]
        .iter()
...
mod tests {
    ...
    fn the_cli_binary_is_not_the_gui() {
        assert!(is_gui_process_name("OpenRoadie.exe"));
        assert!(is_gui_process_name("roadie-desktop.exe"));
        assert!(!is_gui_process_name("roadie.exe")); // the CLI
    }
}
