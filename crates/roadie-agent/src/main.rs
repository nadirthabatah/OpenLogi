//! OpenRoadie background agent — headless, always-on.
//!
...
mod resume_windows;
// The shared locale catalogs live in `roadie-ui`; the negotiation that picks
// one is `roadie_core::locale`. `t!` resolves against a backend each binary
...
fn main() {
    ...
    let _guard = match roadie_core::single_instance::acquire("agent.lock") {
        Ok(g) => g,
        Err(roadie_core::single_instance::InstanceError::AlreadyRunning { path }) => {
            ...
            let Some(g) = takeover::try_replace_stale() else {
                info!(path = %path.display(), "another roadie-agent is already running — exiting");
                return;
            };
    ...
    roadie_core::locale::activate(config.app_settings.language.as_deref());
