...
/// daily-rotated `agent.<date>.log` under [`roadie_core::paths::state_dir`]
...
pub(crate) fn init() {
    let filter = EnvFilter::try_from_env("ROADIE_LOG").unwrap_or_else(|_| EnvFilter::new("info"));
...
fn file_appender() -> io::Result<RollingFileAppender> {
    let dir = roadie_core::paths::state_dir().map_err(io::Error::other)?;
