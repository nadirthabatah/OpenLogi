...
use roadie_agent_core::action_ring::ActionRingManager;
use roadie_agent_core::event_monitor::SharedEventMonitor;
use roadie_agent_core::hardware;
use roadie_agent_core::observable::ObservableState;
use roadie_agent_core::orchestrator::{Orchestrator, SharedRuntime};
use roadie_agent_core::runtime::ActionDispatcher;
use roadie_core::binding::ActionRingSlot;
use roadie_core::config::{Config, Lighting};
use roadie_core::device::DeviceInventory;
use roadie_hid::{
    ...
    async fn reload_config(self, _: Context) -> Result<(), ConfigReloadError> {
        match Config::load_or_default() {
            Ok(config) => {
                ...
                // install.
                roadie_core::locale::activate(language.as_deref());
                #[cfg(target_os = "macos")]
    ...
    async fn set_dpi(self, _: Context, route: DeviceRoute, dpi: Dpi) -> Result<(), WriteError> {
        self.shared
            ...
            .run(HidppOperation::WriteDpi, |c| async move {
                roadie_hid::set_dpi_on(&c, dpi).await
            })
            .await
    }
    ...
    async fn set_lighting(
        ...
        self.shared
            ...
            .run(HidppOperation::Lighting, |c| async move {
                roadie_hid::set_keyboard_color_on(&c, r, g, b).await
            })
            .await
    }
    ...
    async fn set_smartshift(
        ...
        self.shared
            ...
            .run(HidppOperation::WriteSmartShift, |c| async move {
                roadie_hid::set_smartshift_on(&c, status).await
            })
            .await
    }
    ...
    async fn read_dpi(self, _: Context, route: DeviceRoute) -> Result<DpiInfo, WriteError> {
        self.shared
            ...
            .run(HidppOperation::ReadDpiCapabilities, |c| async move {
                roadie_hid::get_dpi_info_on(&c).await
            })
            .await
    }
    ...
    async fn read_smartshift(
        ...
        self.shared
            ...
            .run(HidppOperation::ReadSmartShift, |c| async move {
                roadie_hid::get_smartshift_status_on(&c).await
            })
            .await
    }
        ...
        match tokio::time::timeout(
            ...
            shared
                ...
                .run(HidppOperation::PlayHaptic, |c| async move {
                    roadie_hid::ensure_haptics_armed_on(&c).await
                }),
        )
        .await
        {
    ...
    for attempt in 1..=3u8 {
        ...
        match tokio::time::timeout(
            ...
            shared
                .device(route)
                .run(HidppOperation::PlayHaptic, |c| async move {
                    roadie_hid::play_haptic_on(&c, waveform).await
                }),
        )
        .await
        {
