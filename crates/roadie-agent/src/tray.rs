...
//! Updates) and "Quit OpenRoadie"; the GitHub/help links live in the GUI's own
//! menu bar, not here. Clicks fire on the main thread's AppKit run loop.
//!
//! GUI-directed actions open [`DeeplinkCommand`] `roadie://` URLs which macOS
...
use roadie_core::brand::{self, DeeplinkCommand};
use roadie_core::config::AppIcon;
    ...
    DispatchQueue::main().exec_async(move || {
        ...
        TRAY.with_borrow(|state| {
            if let Some(state) = state.as_ref() {
                status_item::set_png_icon(&state.item, mtm, glyph(icon), "OpenRoadie");
            }
        });
    });
}
...
define_class!(
    ...
    #[name = "OpenRoadieAgentWorkspaceResumeTarget"]
    struct ResumeTarget;
...
define_class!(
    ...
    #[name = "OpenRoadieAgentMenuTarget"]
    struct MenuTarget;

    impl MenuTarget {
        #[unsafe(method(openOpenRoadie:))]
        fn open_roadie(&self, _sender: Option<&AnyObject>) {
        ...
        #[unsafe(method(quitOpenRoadie:))]
        fn quit_roadie(&self, _sender: Option<&AnyObject>) {
            quit_agent();
        }
    }
);
...
/// Route a GUI-directed [`DeeplinkCommand`] through the `roadie://` scheme.
...
/// Whether an OpenRoadie GUI process is currently running (prod or dev bundle).
...
fn install_status_item(
    ...
    status_item::set_png_icon(&status_item, mtm, glyph(app_icon), "OpenRoadie");
...
fn build_menu(mtm: MainThreadMarker, target: &MenuTarget) -> Retained<objc2_app_kit::NSMenu> {
    ...
    if let Some(image) = NSImage::imageWithSystemSymbolName_accessibilityDescription(
        ...
        Some(&NSString::from_str(&rust_i18n::t!("Quit OpenRoadie"))),
    ) {
