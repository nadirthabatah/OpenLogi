...
//!
//! ```sh
//! cargo run -p roadie-agent --bin roadie-agent-mock
//! ROADIE_DEV_AGENT=0 cargo run -p roadie-desktop   # in a second terminal
//! ```
//!
//! It defaults to the `roadie-dev` profile — the one the dev app bundle
//! already uses — so it meets the dev GUI on the dev socket and the installed
//! production app is left alone. `ROADIE_PROFILE=prod` serves the production
...
use roadie_core::binding::ActionRingSlot;
use roadie_core::config::SMARTSHIFT_AUTO_DISENGAGE_DEFAULT;
use roadie_core::config::{Config, Lighting};
...
use roadie_core::single_instance::{self, InstanceError};
use roadie_hid::{
...
fn main() -> ExitCode {
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            EnvFilter::try_from_env("ROADIE_LOG").unwrap_or_else(|_| EnvFilter::new("info")),
        )
        .init();
    ...
    let _guard = match single_instance::acquire("agent.lock") {
        ...
        Err(InstanceError::AlreadyRunning { path }) => {
            warn!(
                path = %path.display(),
                "an roadie-agent is already running — quit it first (pkill -x roadie-agent)"
            );
...
/// Claim the `roadie-dev` profile unless the caller picked one.
///
...
fn default_to_dev_profile() {
    if std::env::var_os("ROADIE_PROFILE").is_some() {
        return;
    }
    #[expect(
        unsafe_code,
        reason = "the profile must be chosen before roadie_core::paths caches it, and only a process-wide env var selects it"
    )]
...
async fn serve(server: MockAgent) -> std::io::Result<()> {
    ...
    info!(
        profile = std::env::var("ROADIE_PROFILE").unwrap_or_default(),
