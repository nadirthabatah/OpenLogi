...
pub fn spawn() {
    ...
    let Ok(directory) = roadie_core::paths::config_dir() else {
    ...
    let result = std::thread::Builder::new()
        .name("roadie-overlay-supervisor".into())
        .spawn(move || {
            let mut spawn = move || {
                Command::new(&binary)
...
fn overlay_binary_path() -> Option<PathBuf> {
    ...
    let sibling = executable
        .parent()?
        .join(format!("roadie-overlay{}", std::env::consts::EXE_SUFFIX));
    if sibling.is_file() {
        return Some(sibling);
    }
    ...
    for app in executable.ancestors().filter(|path| {
        ...
        for relative in [
            "Contents/Library/LoginItems/OpenRoadie Overlay Dev.app/Contents/MacOS/roadie-overlay",
            "Contents/Library/LoginItems/OpenRoadie Overlay.app/Contents/MacOS/roadie-overlay",
            // Bundles built before the helpers were renamed to their display names.
            "Contents/Library/LoginItems/OpenRoadieOverlay.app/Contents/MacOS/roadie-overlay",
        ] {
...
mod tests {
    ...
    fn path_search_returns_none_for_an_impossible_name() {
        assert_eq!(
            find_on_path("roadie-overlay-this-file-does-not-exist"),
            None
        );
    }
    ...
    fn nested_overlay_path_has_expected_layout() {
        let outer = Path::new("/Applications/OpenRoadie.app");
        assert_eq!(
            outer.join(
                "Contents/Library/LoginItems/OpenRoadie Overlay.app/Contents/MacOS/roadie-overlay"
            ),
            Path::new(
                "/Applications/OpenRoadie.app/Contents/Library/LoginItems/OpenRoadie Overlay.app/Contents/MacOS/roadie-overlay"
            )
        );
    }
}
