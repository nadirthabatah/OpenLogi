...
use roadie_agent_core::runtime::hook;
use roadie_agent_core::watchers::foreground_app::ForegroundUpdate;
use roadie_agent_core::watchers::inventory::InventoryEvent;
...
async fn request_input_monitoring() {
    ...
    if !roadie_hid::permissions::has_access() {
        let access_after_prompt = tokio::task::spawn_blocking(|| {
            roadie_hid::permissions::request_access();
            roadie_hid::permissions::has_access()
        })
        .await;
