//! `roadie camera` — read and write device-level UVC image controls.
//!
//! Exercises the `roadie-camera` UVC path (the same primitive the GUI controls
//! panel uses). Changes land in the camera's own registers, so other apps
//! (Google Meet, Zoom, OBS) see them too.

use anyhow::{Result, anyhow};
use clap::{Args, Subcommand};
use roadie_camera::{AutoToggle, CameraControl};
    ...
    match args.cmd.unwrap_or(CameraCmd::Get) {
        ...
        CameraCmd::Set { control, value } => {
            ...
            if let Some(toggle) = AutoToggle::ALL.iter().find(|t| t.name() == raw) {
                roadie_camera::set_auto(&uid, *toggle, value != 0).map_err(|e| anyhow!("{e}"))?;
                println!("set {} = {}", toggle.name(), value != 0);
            } else {
                let control = parse_control(&raw)?;
                roadie_camera::set_control(&uid, control, value).map_err(|e| anyhow!("{e}"))?;
