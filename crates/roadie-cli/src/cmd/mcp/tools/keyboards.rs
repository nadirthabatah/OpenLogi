...
//! `roadie_hid::via`, and the result of a change always names the key that
//! was there before, so the model can offer to put it back.

use roadie_hid::via::{self, Attached, Session};
use roadie_via::keycode;
...
pub async fn read_keymap(arguments: &Value) -> Result<String, String> {
    ...
    let (rows, rows_capped) =
        roadie_hid::via::scan_edge(optional_number(arguments, "rows")?.unwrap_or(6));
    let (columns, columns_capped) =
        roadie_hid::via::scan_edge(optional_number(arguments, "columns")?.unwrap_or(16));
    ...
    if rows_capped || columns_capped {
        result.insert(
            ...
            json!(format!(
                ...
                edge = roadie_hid::via::MAX_SCAN_EDGE
            )),
        );
    }
...
mod tests {
    ...
    fn every_key_the_undo_instruction_can_name_resolves_again() {
        for code in [
            ...
            let printed = roadie_via::keycode::describe(code);
            assert_eq!(
                resolve(&printed),
                Ok(code),
