//! `roadie diag controls` — dump HID++ reprogrammable controls.

use std::fmt;

use anyhow::{Context, Result};
use clap::Args;
use roadie_hid::ReprogControlEntry;
...
pub async fn run(args: ControlsArgs) -> Result<()> {
    ...
    let controls = roadie_hid::dump_reprog_controls(&route)
        .await
...
mod tests {
    ...
    fn summarizes_empty_capabilities() {
        let entry = ReprogControlEntry {
            cid: 0,
            task_id: 0,
            flags: roadie_hid::reprog_controls::CidFlags::default(),
        };
    ...
    fn summarizes_haptic_relevant_capabilities() {
        let entry = ReprogControlEntry {
            cid: 0,
            task_id: 0,
            flags: roadie_hid::reprog_controls::CidFlags::DIVERTABLE
                | roadie_hid::reprog_controls::CidFlags::FORCE_RAW_XY
                | roadie_hid::reprog_controls::CidFlags::ANALYTICS_KEY_EVENTS,
        };
