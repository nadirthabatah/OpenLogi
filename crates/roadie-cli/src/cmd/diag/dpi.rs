//! `roadie diag dpi` — DPI write round-trip.

use std::fmt;

use anyhow::{Context, Result};
use clap::Args;
use roadie_hid::DpiCapabilities;
...
pub async fn run(args: DpiArgs) -> Result<()> {
    ...
    let info = roadie_hid::get_dpi_info(&route)
        .await
    ...
    roadie_hid::set_dpi(&route, target)
        .await
        .context("write DPI")?;

    let after = roadie_hid::get_dpi(&route)
        .await
    ...
    roadie_hid::set_dpi(&route, before)
        .await
        .context("restore DPI")?;
