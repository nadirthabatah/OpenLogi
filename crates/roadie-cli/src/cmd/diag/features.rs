//! `roadie diag features` — dump the device's HID++ feature table.
//!
...
use anyhow::Result;
use clap::Args;
use roadie_hid::{DeviceRoute, FeatureType, FirmwareEntity};
...
pub async fn run(_args: FeaturesArgs) -> Result<()> {
    ...
    for inv in &inventories {
        for paired in inv.paired.iter().filter(|p| p.online) {
            ...
            match roadie_hid::dump_features(&route).await {
                Ok(entries) => {
            ...
            match roadie_hid::dump_firmware_entities(&route).await {
                Ok(entries) => {
