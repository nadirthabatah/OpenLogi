//! `roadie diag smartshift` — SmartShift toggle round-trip.

use std::num::NonZeroU8;

use anyhow::{Context, Result};
use clap::Args;
use roadie_hid::{SmartShiftAutoDisengage, TunableTorque};
...
pub async fn run(args: SmartshiftArgs) -> Result<()> {
    ...
    if let Some(n) = args.sensitivity {
        ...
        let before = roadie_hid::get_smartshift_status(&route)
            .await
        ...
        let after = roadie_hid::set_smartshift_sensitivity(&route, requested)
            .await
    ...
    let new_mode = roadie_hid::toggle_smartshift(&route)
        .await
        .context("toggle SmartShift")?;
    println!("  toggled to: {new_mode:?}");

    let after = roadie_hid::get_smartshift_status(&route)
        .await
    ...
    roadie_hid::toggle_smartshift(&route)
        .await
