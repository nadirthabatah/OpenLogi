...
//!
//! `roadie mcp` turns this process into a local MCP server speaking
...
//! naming the command `roadie mcp`; no network port is opened and nothing
...
//!
//! [`ClientKind::Cli`]: roadie_ipc::ClientKind::Cli
...
/// Arguments for `roadie mcp`. None yet: the server's tool surface is
...
fn initialize_result(params: &Value) -> Value {
    ...
    serde_json::json!({
        ...
        "instructions": "Local control of the peripherals managed by the \
            OpenRoadie agent on this machine. Start with list_devices: its \
