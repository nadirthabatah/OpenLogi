//! `roadie assets sync` — pull every device's bundle-required files
//! into the roadie-desktop crate so `cargo bundle` can pick them up.
//!
//! Probes OpenRoadie's asset mirrors, fetches `index.json`, writes per-device
...
//! `roadie assets sync && cargo bundle --release`.
...
use clap::Args;
use roadie_assets::{AssetRegistry, FRONT_RENDER_FILES, FetchOutcome, METADATA_FILES, http};

/// Returns true when `name` is an *optional* asset OpenRoadie fetches when the
...
pub struct SyncArgs {
    ...
    #[arg(long, env = "ROADIE_ASSETS")]
    base: Option<String>,
    /// Destination directory. Default matches the cargo-bundle
    /// resources path declared in roadie-desktop/Cargo.toml.
    #[arg(long, default_value = "crates/roadie-desktop/assets")]
    out: PathBuf,
}
...
pub fn run(args: SyncArgs) -> Result<()> {
    ...
    for depot in depots {
        ...
        let wanted: Vec<&roadie_assets::FileEntry> = entry
            .files
            .iter()
