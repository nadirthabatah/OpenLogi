//! `roadie diag` — real-device smoke tests for the HID++ write path.
//!
...
//! same `roadie_hid` API the GPUI app uses, so a green diag means the
//! GUI's write path works on this host.

use anyhow::{Result, anyhow};
use clap::Subcommand;
use roadie_hid::{DeviceRoute, dump_features};
