//! OpenRoadie CLI implementation. The `roadie` binary is a thin wrapper that
...
/// OpenRoadie: a local-first companion for Logitech HID++ peripherals.
#[derive(Debug, Parser)]
#[command(
    name = "roadie",
    version,
    about = "OpenRoadie: a local-first companion for Logitech HID++ peripherals.",
    long_about = None,
)]
struct Cli {
    #[command(subcommand)]
...
mod tests {
    ...
    /// A bare `roadie` invocation must remain valid — `run()` defaults the
    /// missing subcommand to `list`.
    #[test]
    fn bare_invocation_has_no_subcommand() {
        let cli = Cli::try_parse_from(["roadie"]).expect("bare invocation parses");
        assert!(cli.cmd.is_none());
    }

    /// A bare `roadie backlight` must stay valid — `run` treats a missing
    ...
    fn backlight_defaults_to_status_and_accepts_a_device_filter() {
        let cli = Cli::try_parse_from(["roadie", "backlight", "--device", "MX KEYS S"])
    ...
    fn backlight_off_is_parsed_as_its_own_action() {
        let cli =
            Cli::try_parse_from(["roadie", "backlight", "off"]).expect("backlight off parses");
    ...
    fn backlight_rejects_an_unknown_action() {
        let result = Cli::try_parse_from(["roadie", "backlight", "dim"]);
    ...
    fn smartshift_rejects_zero_sensitivity() {
        ...
        let result = Cli::try_parse_from(["roadie", "diag", "smartshift", "--sensitivity", "0"]);
    ...
    fn lighting_color_is_positional_and_method_is_a_flag() {
        let cli = Cli::try_parse_from([
            "roadie", "diag", "lighting", "ff0000", "--method", "effects",
        ])
    ...
    fn lighting_rejects_unknown_method() {
        let result =
            Cli::try_parse_from(["roadie", "diag", "lighting", "ff0000", "--method", "bogus"]);
