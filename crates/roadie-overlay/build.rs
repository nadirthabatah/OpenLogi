//! Build script for roadie-overlay: embeds the Windows exe resources — the
...
//!
//! Kept in sync with its twins in `crates/roadie-agent/build.rs` and
//! `crates/roadie-desktop/build.rs` — only the description/filename strings
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "FileDescription", "OpenRoadie Actions Ring Overlay"
            VALUE "FileVersion", "{version}"
            VALUE "InternalName", "roadie-overlay"
            VALUE "OriginalFilename", "roadie-overlay.exe"
            VALUE "ProductName", "OpenRoadie"
...
"#
    ...
    let rc_path = out.join("roadie-overlay.rc");
