...
// both binaries expand `i18n!` over the one catalog in `roadie-ui` — the same
...
fn main() -> Result<()> {
    ...
    roadie_core::locale::activate(None);
    ...
    let mut app = gpui_platform::application().with_assets(roadie_ui::action_icons::ActionIcons);
    ...
    app.run(move |cx| {
        ...
        cx.spawn(async move |cx| {
            while let Some(observed) = invocations.recv().await {
                ...
                roadie_core::locale::activate(invocation.language.as_deref());
                cx.update(|cx| {
                    for handle in cx.windows() {
                        let _ = handle.update(cx, |_, window, _| window.remove_window());
                    }
...
mod tests {
    ...
    /// The catalog this binary translates against lives in `roadie-ui` and is
