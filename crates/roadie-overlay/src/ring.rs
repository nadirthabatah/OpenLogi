...
use roadie_ipc::ActionRingInvocation;
use roadie_ui::action_icons::RING_CANCEL_ICON;
use roadie_ui::color;
use std::sync::Arc;
use tokio::sync::mpsc;
...
/// shared (`roadie_ui::color`); these greys are local by nature.
...
pub(crate) fn ring_window_options(cx: &mut gpui::App) -> WindowOptions {
    let cursor = roadie_hook::cursor_position();
    ...
    WindowOptions {
        ...
        app_id: Some("roadie-action-ring".to_string()),
