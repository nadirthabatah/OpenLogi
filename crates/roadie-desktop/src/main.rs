//! OpenRoadie's desktop app: process bootstrap.
//!
...
// Loads the Crowdin-managed `crates/roadie-ui/locales/*.yml` files at compile
...
rust_i18n::i18n!("../roadie-ui/locales", fallback = "en");

use anyhow::Result;
use roadie_core::brand::DeeplinkCommand;
use roadie_core::config::{Config, ConfigFile};
...
fn main() -> Result<()> {
    ...
    if std::env::var_os("ROADIE_COMPONENT_GALLERY").is_some_and(|value| value == "1") {
        ui::gallery::run();
        return Ok(());
    }

    let _guard = match roadie_core::single_instance::acquire("roadie.lock") {
        Ok(g) => g,
        Err(roadie_core::single_instance::InstanceError::AlreadyRunning { path }) => {
            info!(
                path = %path.display(),
                "another OpenRoadie instance is already running — exiting"
            );
            return Ok(());
        }
    ...
    // URL scheme: `open roadie://open-settings` from the agent's tray or
    ...
    app.on_open_urls(move |urls| {
        for url in &urls {
            if let Some(cmd) = DeeplinkCommand::parse_url(url) {
                ...
                warn!(url, "unknown roadie:// command — ignoring");
            }
        }
    });
