...
impl AppState {
    ...
    /// The active device's persisted wheel resolution, or `None` when OpenRoadie
    /// leaves the device default untouched.
    #[must_use]
    pub fn current_scroll_resolution(&self) -> Option<roadie_core::config::ScrollResolution> {
    ...
    /// it. `None` removes OpenRoadie's override. No-op without a selected,
    /// HiResWheel-capable device.
    pub fn commit_scroll_resolution(
        &mut self,
        resolution: Option<roadie_core::config::ScrollResolution>,
    ) {
...
#[cfg(test)]
mod tests {
    use roadie_core::config::{Config, DeviceConfig, LinkConfig, LinkOverrides};
    use roadie_core::device::{Capabilities, DeviceKind};
