...
use roadie_core::config::{Config, DeviceIdentity, canonical_device_key};
...
/// [`PairedDevice`](roadie_core::device::PairedDevice) so the gallery can
...
pub(super) fn build_device_list(
    ...
    if std::env::var_os("ROADIE_DEMO_KEYBOARD").is_some() {
...
fn append_standalone(
    ...
    for device in devices {
        ...
        let stable_id = DeviceStableId::from_parts(
            route.as_ref(),
            roadie_core::hid::DIRECT_DEVICE_INDEX,
...
/// render without the hardware. Gated behind the `ROADIE_DEMO_KEYBOARD` env
...
/// `resolve_device_kind` (`crates/roadie-hid/src/inventory/mappings.rs`); this applies
...
#[cfg(test)]
mod tests {
    use roadie_core::config::{Config, DeviceConfig, LinkConfig};
    use roadie_core::device::{
