...
use roadie_camera::CameraControl;
...
impl AppState {
    ...
    pub(crate) fn request_camera_access(cx: &mut gpui::App) {
        ...
        roadie_camera::request_camera_access();
        Self::update(cx, |state, cx| {
            ...
            state.camera_permission_poll = Some(cx.spawn(async move |state, cx| {
                for _ in 0..TICKS_MAX {
                    ...
                    if roadie_camera::camera_authorization()
                        != roadie_camera::CameraAuthorization::Undetermined
                    {
                        break;
                    }
                }
    ...
    pub fn has_camera(&self) -> bool {
        self.devices
            .records
            .iter()
            .any(|r| matches!(r.kind, roadie_core::device::DeviceKind::Camera))
    }
    ...
    #[must_use]
    pub fn camera_auto(&self, config_key: &str, toggle: roadie_camera::AutoToggle) -> Option<bool> {
        self.config
            .camera_controls(config_key)?
            .0
            .get(toggle.name())
    ...
    pub fn commit_camera_auto(
        &mut self,
        config_key: &str,
        toggle: roadie_camera::AutoToggle,
        on: bool,
    ) {
    ...
    pub fn camera_profiles(
        &self,
        config_key: &str,
    ) -> std::collections::BTreeMap<String, roadie_core::config::CameraControls> {
    ...
    pub fn save_camera_profile(
        ...
        config_key: &str,
        name: &str,
        snap: roadie_core::config::CameraControls,
    ) {
        self.config
