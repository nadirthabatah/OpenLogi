...
use roadie_core::config::LightSettings;
use roadie_core::hid::{DeviceRoute, LightCommand, WriteError};
use tracing::debug;
...
impl AppState {
    ...
    pub fn commit_light(&mut self, light: LightSettings) {
        ...
        let commands = capabilities.map_or_else(Vec::new, |capabilities| {
            roadie_core::hid::commands_for_light_settings(effective, capabilities)
        });
