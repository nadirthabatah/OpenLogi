...
use roadie_camera::Camera;
use roadie_core::binding::{Action, ActionRingIcon, ActionRingSlot, Binding, ButtonId, RingAction};
use roadie_core::config::{
    Config, DeviceIdentity, LightSettings, Lighting, ScrollResolution, ThumbwheelSensitivity,
    VerticalScrollSensitivity,
};
use roadie_core::device::{
...
fn agent_reload_error_stays_visible_until_a_successful_confirmation() {
    ...
    assert!(
        state.apply_config_reload_result(Err(roadie_ipc::ConfigReloadError {
...
fn direct_inventory(unit_id: [u8; 4]) -> DeviceInventory {
    DeviceInventory {
        ...
        paired: vec![PairedDevice {
            slot: roadie_core::hid::DIRECT_DEVICE_INDEX,
...
) -> (roadie_core::hid::LightCommand, u64) {
...
fn the_profile_shown_is_the_apps_even_while_this_window_has_focus() {
    // The frontmost application is OpenRoadie whenever the user is looking at
    ...
    state.set_foreground(ForegroundApps {
        current: Some(app(roadie_core::brand::APP_ID, "OpenRoadie")),
...
fn camera_controls(brightness: i32) -> roadie_core::config::CameraControls {
    roadie_core::config::CameraControls(std::collections::BTreeMap::from([(
        "brightness".into(),
        brightness,
    )]))
}
...
fn light_write_failure_reaches_the_gui_state() {
    ...
    let Ok(crate::services::ipc::Command::SetLight(
        _,
        roadie_core::hid::LightCommand::Power(false),
        _,
        request_id,
    )) = receiver.try_recv()
    else {
    ...
    let Ok(crate::services::ipc::Command::SetLight(
        _,
        roadie_core::hid::LightCommand::BrightnessPercent(50),
        _,
...
fn superseded_light_write_keeps_prior_successes_for_reconciliation() {
    ...
    assert_eq!(first_power, roadie_core::hid::LightCommand::Power(false));
    assert_eq!(
        first_brightness,
        roadie_core::hid::LightCommand::BrightnessPercent(40)
    );
    ...
    assert_eq!(second_power, roadie_core::hid::LightCommand::Power(true));
    assert_eq!(
        second_brightness,
        roadie_core::hid::LightCommand::BrightnessPercent(60)
    );
    ...
    assert!(state.apply_light_command_result(
        key.clone(),
        second_request_id,
        roadie_core::hid::LightCommand::Power(true),
        Ok(()),
    ));
    assert!(state.apply_light_command_result(
        key.clone(),
...
fn transient_light_state_is_kept_in_memory_and_only_supported_commands_are_sent() {
    ...
    assert!(matches!(
        ...
        Ok(crate::services::ipc::Command::SetLight(
            _,
            roadie_core::hid::LightCommand::BrightnessPercent(37),
            _,
            _
        ))
    ));
...
fn gesture_maps_cover_every_gesture_mode_button() {
    ...
    use roadie_core::binding::{ButtonId, GestureDirection};
