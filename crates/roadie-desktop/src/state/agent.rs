...
pub(super) struct AgentSession {
    ...
    monitor_events: std::collections::VecDeque<roadie_ipc::MonitorEvent>,
    #[cfg(all(target_os = "macos", debug_assertions))]
    event_taps: Vec<roadie_hook::EventTapInfo>,
}
...
impl AppState {
    ...
    #[cfg(all(target_os = "macos", debug_assertions))]
    pub fn push_monitor_events(&mut self, events: Vec<roadie_ipc::MonitorEvent>) {
        const MAX: usize = 200;
    ...
    #[cfg(all(target_os = "macos", debug_assertions))]
    #[must_use]
    pub fn monitor_events(&self) -> &std::collections::VecDeque<roadie_ipc::MonitorEvent> {
    ...
    #[cfg(all(target_os = "macos", debug_assertions))]
    pub fn set_event_taps(&mut self, taps: Vec<roadie_hook::EventTapInfo>) {
        self.agent.event_taps = taps;
    }
    /// The cached event-tap snapshot for the Diagnostics page.
    #[cfg(all(target_os = "macos", debug_assertions))]
    #[must_use]
    pub fn event_taps(&self) -> &[roadie_hook::EventTapInfo] {
    ...
    #[must_use]
    pub fn agent_status(&self) -> Option<&roadie_ipc::AgentStatus> {
