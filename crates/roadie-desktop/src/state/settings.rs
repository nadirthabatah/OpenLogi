...
impl AppState {
    ...
    /// Persist the chosen theme name for one mode (`None` = the OpenRoadie brand
    ...
    /// Whether OpenRoadie manages `key` (capture + volatile re-apply).
    #[must_use]
    ...
    /// Enable or disable OpenRoadie's management of `key` and persist it. The
    ...
    /// process-global locale via [`roadie_core::locale`], and repaint open UI.
    /// No-op when unchanged.
    pub fn set_language(&mut self, language: Option<String>, cx: &mut Context<Self>) {
        ...
        roadie_core::locale::activate(self.config.app_settings.language.as_deref());
