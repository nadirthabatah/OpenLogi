...
//!
//! Translations live in `crates/roadie-ui/locales/*.yml` and are loaded at
...
//! change — `roadie_core::locale`'s parity test enforces key-for-key match,
...
//! [`roadie_core::locale`], shared with the overlay helper.

use roadie_core::config::AppSettings;
use roadie_core::locale::activate;
...
#[cfg(test)]
mod tests {
    use roadie_core::binding::{Action, ButtonId, GestureDirection};
    ...
    fn locale_file_resolves_keys() {
        ...
        const BLURB: &str = "OpenRoadie captures mouse buttons (Back / Forward / gesture button) through the system Accessibility permission and runs the actions you bind. Features that talk to the device directly — DPI, SmartShift — are unaffected.";
        ...
        assert_eq!(rust_i18n::t!("Quit OpenRoadie"), "退出 OpenRoadie"); // menu-bar status item
        ...
        assert_eq!(
            rust_i18n::t!("Automatically start OpenRoadie when you log in to macOS."),
            "登录 macOS 时自动启动 OpenRoadie。"
        );
        assert_eq!(
