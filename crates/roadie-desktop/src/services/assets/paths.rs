...
use roadie_assets::{INDEX_NAME, Index};
use tracing::{debug, warn};

/// Per-user writable cache root: `roadie_core::paths::data_dir()` plus an
...
pub(super) fn user_cache_root() -> PathBuf {
    roadie_core::paths::data_dir().map_or_else(|_| PathBuf::from("./assets"), |d| d.join("assets"))
}
