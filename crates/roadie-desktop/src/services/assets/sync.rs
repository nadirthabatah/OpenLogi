//! Background sync against OpenRoadie's asset mirrors.
//!
...
use roadie_assets::http;
use roadie_assets::{
    AssetRegistry, AssetSource, BUTTONS_RENDER_FILES, DepotManifest, DeviceEntry, FetchOutcome,
};
use roadie_core::config::AssetSourcePreference;
use roadie_core::device::DeviceModelInfo;
use tracing::{debug, info, warn};

/// Whether the startup HTTP sync should run on this launch.
///
/// Policy:
/// - `ROADIE_SYNC=off` → never run.
/// - `ROADIE_SYNC=on` → always run.
...
pub fn should_run(has_bundle: bool) -> bool {
    match std::env::var("ROADIE_SYNC").ok().as_deref() {
...
pub fn load_registry(source: Option<AssetSource>) -> Result<AssetRegistry> {
    ...
    // `ROADIE_FORCE_DEPOT` names a depot with no device behind it, so it has
    // no target to ride in on. It belongs to whoever loads the registry — and
    // `ext = 0` gets its base PNG.
    if let Ok(forced) = std::env::var("ROADIE_FORCE_DEPOT")
...
pub(crate) fn source_segment(preference: AssetSourcePreference) -> &'static str {
    match preference {
        ...
        AssetSourcePreference::OpenRoadie => "roadie",
...
fn source_for_sync(
    ...
    match preference {
        ...
        AssetSourcePreference::OpenRoadie => Some(AssetSource::Production),
...
mod tests {
    ...
    #[test]
    fn roadie_preference_uses_the_official_source() {
        assert_eq!(
            source_for_sync(AssetSourcePreference::OpenRoadie, None),
