...
//!    packaging time by `roadie assets sync` and shipped with every
//!    release. Zero network at end-user runtime.
//! 2. The per-user cache at `~/.local/share/roadie/assets/` —
...
use roadie_assets::http::safe_component_path;
use roadie_assets::{
    BUTTONS_RENDER_FILES, DeviceEntry, FRONT_RENDER_FILES, Index, METADATA_FILES, Metadata,
};
use roadie_core::device::{DeviceKind, DeviceModelInfo};
...
///
/// Match order:
/// 1. `ROADIE_FORCE_DEPOT` env override (dev convenience).
...
pub(crate) fn resolve_in_index<'a>(
    ...
    if let Ok(forced) = std::env::var("ROADIE_FORCE_DEPOT")
        && let Some((depot, entry)) = index
            ...
            .map(|(d, e)| (d.as_str(), e))
    {
        debug!(depot, "ROADIE_FORCE_DEPOT override active");
...
mod tests {
    use super::*;
    use roadie_assets::DeviceEntry;
    use roadie_core::device::DeviceTransports;
        ...
        let resolver = AssetResolver {
            ...
            index: Some(index_of(
                ...
                DeviceEntry {
                    ...
                    kind: "ILLUMINATION_LIGHT".into(),
                    asset_path: "v1/devices/litra_glow/".into(),
                    files: vec![roadie_assets::FileEntry {
                        name: "front.png".into(),
                        sha256: String::new(),
                        bytes: 0,
                    }],
