...
use roadie_core::hid::{Dpi, SmartShiftStatus};
use tokio::sync::mpsc;
use tracing::warn;
...
impl AppState {
    ...
    pub fn apply_config_reload_result(
        &mut self,
        result: Result<(), roadie_ipc::ConfigReloadError>,
    ) -> bool {
    ...
    ///
    /// Not [`roadie_ipc::ForegroundApps::current`]: while this window has
    /// focus *OpenRoadie* is the frontmost application, so the app the user means
    /// is the one they came from. The recent list is exactly that — it excludes
    /// OpenRoadie's own processes, so its head is the frontmost application
