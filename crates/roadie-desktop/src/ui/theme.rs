//! OpenRoadie UI theme and shared sizes.
//!
//! Two layers:
//!
...
//!   with the OS appearance (the OpenRoadie accent blue, the connectivity dots).
...
use roadie_core::config::{Appearance, UiScale};

use crate::state::AppState;

use super::spacing::DynamicSpacing;

// The brand accent lives in `roadie-ui` so the overlay paints the same blue
...
pub use roadie_ui::color::{ACCENT_BLUE, accent};
...
/// framework widgets read. The bundled "OpenRoadie" theme (`themes/roadie.json`)
...
const ROADIE_THEME_JSON: &str = include_str!("../../themes/roadie.json");
...
pub const ROADIE_LIGHT: &str = "OpenRoadie Light";
pub const ROADIE_DARK: &str = "OpenRoadie Dark";
...
pub fn register_builtin_themes(cx: &mut App) {
    ...
    for json in std::iter::once(ROADIE_THEME_JSON).chain(UPSTREAM_THEME_JSON.iter().copied()) {
...
pub fn apply_from_settings(window: Option<&mut Window>, cx: &mut App) {
    ...
    let (light, dark) = {
        ...
        (
            pick(light_name.as_deref(), ROADIE_LIGHT),
            pick(dark_name.as_deref(), ROADIE_DARK),
        )
    };
    {
...
mod tests {
    ...
    #[test]
    fn roadie_theme_text_pairs_meet_normal_text_contrast() {
        let Ok(theme_set) = serde_json::from_str::<serde_json::Value>(ROADIE_THEME_JSON) else {
            panic!("OpenRoadie theme JSON should parse");
        };
        let Some(themes) = theme_set["themes"].as_array() else {
            panic!("OpenRoadie theme JSON should contain themes");
        };
