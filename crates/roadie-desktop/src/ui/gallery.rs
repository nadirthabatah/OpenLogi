...
use roadie_core::config::UiScale;
use roadie_core::device::{BatteryInfo, BatteryLevel, BatteryStatus};
...
use super::theme::{self, ContentWidth, Palette, ROADIE_DARK, ROADIE_LIGHT, Typography as _};

const TITLE: &str = "OpenRoadie Component Gallery";
...
fn install_roadie_themes(cx: &mut App) {
    let (light, dark) = {
        let registry = ThemeRegistry::global(cx);
        (
            registry.themes().get(ROADIE_LIGHT).cloned(),
            registry.themes().get(ROADIE_DARK).cloned(),
        )
    };
...
mod tests {
    ...
    fn gallery_renders_without_application_state(cx: &mut TestAppContext) {
        ...
        cx.update(install_roadie_themes);
