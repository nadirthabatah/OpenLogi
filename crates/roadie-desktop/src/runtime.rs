...
//! downloads, and `roadie://` deeplinks.
...
use roadie_core::config::{AssetSourcePreference, Config};
use roadie_core::device::{DeviceInventory, StandaloneDevice};
use roadie_ipc::AgentSnapshot;
...
pub(crate) struct Startup {
    ...
    /// `roadie://` URLs, from the tray or another app.
...
pub(crate) fn spawn(startup: Startup, cx: &mut gpui::App) {
    cx.spawn(async move |cx| {
        ...
        let cams = cx
            .background_executor()
            .spawn(async { roadie_camera::enumerate_cameras() })
            .await;
...
impl Runtime {
    ...
    fn apply_snapshot(&mut self, snapshot: &AgentSnapshot, cx: &AsyncApp) {
        ...
        // shrink it.
        let inventory_ready = snapshot.status.inventory == roadie_ipc::InventoryHealth::Ready;
        if inventory_ready {
...
mod tests {
    ...
    fn a_source_change_rekeys_every_subscription() {
        ...
        for watch in &watches {
            assert_ne!(
                watch_key(AssetSourcePreference::OpenRoadie, watch),
