...
// The fallback deliberately points at this fork's own releases, not at
// upstream's `updates.openlogi.org`: a fork that phones upstream's update
// channel by default would, at best, report upstream's versions as updates
// for itself. (Even then it could not install one — a build without the
// fork's minisign key fails closed, and a build with it rejects upstream's
...
const MINISIGN_PUBLIC_KEY: Option<&str> = option_env!("ROADIE_UPDATE_MINISIGN_PUBLIC_KEY");
