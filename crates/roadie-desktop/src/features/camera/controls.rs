...
use roadie_camera::{AutoToggle, CameraControl, CameraState, ControlRange};
use roadie_core::config::CameraControls;
use tracing::debug;
...
impl CameraControlsPanel {
    ...
    fn active_camera(cx: &Context<Self>) -> Option<(String, String)> {
        ...
        if !matches!(record.kind, roadie_core::device::DeviceKind::Camera) {
            return None;
        }
    ...
    fn reapply_saved(
        ...
        let Err(e) = roadie_camera::apply_settings(uid, apply_autos, apply_values) else {
            return Reapplied::Clean;
        };
        ...
        match roadie_camera::read_camera_state(uid) {
    ...
    fn ensure_built(&mut self, key: &str, uid: &str, cx: &mut Context<Self>) {
        ...
        let Ok(snap) = roadie_camera::read_camera_state(uid) else {
    ...
    fn commit_release(
        ...
        let written = match takeover {
            Some((toggle, _)) => {
                roadie_camera::apply_settings(uid, &[(toggle, false)], &[(control, v)])
            }
            None => roadie_camera::set_control(uid, control, v),
        };
    ...
    fn toggle_auto(&mut self, ix: usize, cx: &mut Context<Self>) {
        ...
        if let Err(e) = roadie_camera::apply_settings(&uid, &[(toggle, on)], &values) {
    ...
    fn reset(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let values: Vec<(CameraControl, i32)> = self
            ...
            .collect();
        if let Err(e) = roadie_camera::apply_settings(&uid, &autos, &values) {
    ...
    fn reset_control(&mut self, ix: usize, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Err(e) = roadie_camera::apply_settings(&uid, &autos, &[(control, default)]) {
