...
use roadie_camera::{CameraAuthorization, CameraStream, Frame};
...
impl CameraPreview {
    pub fn new(cx: &mut Context<Self>) -> Self {
        let permission_obs = cx.subscribe(
            ...
            |preview, _, event: &StateEvent, cx| {
                ...
                if preview.awaiting_access && roadie_camera::camera_access_granted() {
    ...
    pub fn set_target(&mut self, target: Option<String>, cx: &mut Context<Self>) {
        if target == self.streaming_uid {
            if self.awaiting_access && roadie_camera::camera_access_granted() {
        ...
        if roadie_camera::camera_access_granted() {
            self.start_stream(cx);
        } else {
    ...
    fn start_stream(&mut self, cx: &mut Context<Self>) {
        ...
        self.stream = roadie_camera::start_stream(uid).ok();
...
impl Render for CameraPreview {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let granted = roadie_camera::camera_access_granted();
        ...
        let capture_supported = !show_placeholder || roadie_camera::capture_supported();
        let authorization_undetermined = show_placeholder
            && capture_supported
            && !granted
            && matches!(
                roadie_camera::camera_authorization(),
