...
fn current_taps(cx: &App) -> Vec<roadie_hook::EventTapInfo> {
    #[cfg(debug_assertions)]
    {
...
#[cfg(debug_assertions)]
fn format_monitor_event(event: &roadie_ipc::MonitorEvent) -> String {
...
#[cfg(debug_assertions)]
fn debug_tap_list(taps: &[roadie_hook::EventTapInfo], pal: Palette) -> impl IntoElement {
    let mut col = v_flex().w_full().mt_2().gap_1().child(
        div()
            .text_caption()
            .text_color(pal.text_muted)
