...
#[cfg(any(target_os = "macos", target_os = "linux"))]
use roadie_permissions as permissions;
    ...
    let page = {
        let mut group = SettingGroup::new()
            ...
            .item(permission_item(
                "perm-bluetooth",
                tr!("Bluetooth"),
                tr!("Allows OpenRoadie to use CoreBluetooth (not required for HID access)."),
            ...
            SettingField::render(move |_, _, cx| {
                ...
                let hint = match status {
                    PermissionStatus::Denied => Some(tr!(
                        "OpenRoadie needs write access to /dev/uinput (for button \
                         remapping) and read/write access to /dev/hidraw* (for HID++ \
                         communication). Install the OpenRoadie udev rules to grant \
