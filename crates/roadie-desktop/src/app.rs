...
use roadie_core::device::{Capabilities, DeviceInventory, DeviceKind};
use roadie_ipc::InventoryHealth;
use tracing::info;
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .child(tr!(
                        "OpenRoadie captures mouse buttons (Back / Forward / gesture button) \
            ...
            .child(
                div()
                    ...
                    .text_color(pal.text_muted)
                    .child(tr!(
                        "Enable “OpenRoadie Agent” in the Accessibility list — the \
                         background agent owns the mouse hook, not the OpenRoadie app. \
...
fn request_accessibility(cx: &mut App) {
    use roadie_permissions::{self as permissions, Permission};
    // Ask the *agent* to fire the prompt (it owns the hook, so the system dialog
    // must name and authorize roadie-agent — prompting in the GUI would grant
...
impl Render for AppView {
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if let Some(issue) = config_issue {
            window.set_window_title("OpenRoadie");
            return root
