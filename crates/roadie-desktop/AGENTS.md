# roadie-desktop — the settings app
...
`roadie-desktop` is a **pure IPC client**. The agent owns the input hook and
...
call in `roadie-ipc` plus a handler in `roadie-agent`/`roadie-agent-core`
— never an `roadie-hid` call from here, and never a "just this once" direct
read. Wire changes are append-only and versioned; see
[`crates/roadie-ipc/AGENTS.md`](../roadie-ipc/AGENTS.md).

Two more things this crate is not:

- **Not the overlay's parent.** `roadie-overlay` is a *sibling* process. It
  links `roadie-ui`, never this crate. Anything both frontends need moves into
  `roadie-ui` — and every dependency added there is added to the overlay too,
  which is why `gpui-component` is not one of them.
- **Not a home for shared presentation.** Ring geometry, the shared asset
  source, locale negotiation: those are `roadie-ui`'s. What only the settings
...
| `runtime.rs` | Everything the app does that isn't a render: one task, one `select!` arm per source that can change long-lived state (agent updates, camera scan, asset commands, finished downloads, `roadie://` deeplinks). |
...
| `app_assets.rs` | The GPUI asset source, composed in order: embedded logo → `roadie-ui`'s `action-icons/` → gpui-component's bundled lucide set. A new icon path that resolves nowhere renders blank rather than failing to build. |
...
- `cargo run -p roadie-desktop` — a cargo runner wraps the build into
  `target/dev/OpenRoadie.app` with the identity, helpers and plist tables
...
- The window shows only the empty state unless an agent is running. With no
  hardware, `cargo run -p roadie-agent --bin roadie-agent-mock` serves a
  scripted inventory.
- `ROADIE_COMPONENT_GALLERY=1` on a debug build opens `ui/gallery.rs` instead
...
- `build.rs` asks `cargo metadata` where gpui-component's source actually sits
  ...
  `ROADIE_THEMES_DIR` overrides the lookup.
- `themes/roadie.json` is the app's own theme, layered on that upstream set.
- `bundle/` holds `OpenRoadie.entitlements` and one `Info.plist` per bundled
...
| anything crossing the agent boundary | [`crates/roadie-ipc/AGENTS.md`](../roadie-ipc/AGENTS.md) |
| `platform/**` or any macOS FFI | [`.claude/rules/objc-ffi.md`](../../.claude/rules/objc-ffi.md) |
| a permission symptom or the bundle identity | [`.claude/skills/roadie-macos-permissions/SKILL.md`](../../.claude/skills/roadie-macos-permissions/SKILL.md) |
