//! Build script for roadie-desktop.
//!
...
//!
//! `ROADIE_THEMES_DIR` overrides the lookup with an explicit path to the
...
fn main() {
    println!("cargo:rerun-if-env-changed=ROADIE_UPDATE_MANIFEST_URL");
    println!("cargo:rerun-if-env-changed=ROADIE_THEMES_DIR");
...
///
/// Kept in sync with the twin in `crates/roadie-agent/build.rs` — only the
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "FileDescription", "OpenRoadie"
            VALUE "FileVersion", "{version}"
            VALUE "InternalName", "roadie-desktop"
            VALUE "OriginalFilename", "OpenRoadie.exe"
...
"#
    ...
    let out = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));
    let rc_path = out.join("roadie-desktop.rc");
...
/// `themes/roadie.json` lives in this repo, so nothing is filtered out here.
...
fn locate_themes_dir() -> PathBuf {
    if let Some(dir) = env::var_os("ROADIE_THEMES_DIR") {
    ...
    panic!(
        "could not locate the gpui-component themes dir via `cargo metadata`; \
         set ROADIE_THEMES_DIR to the gpui-component `themes/` directory"
    );
}
