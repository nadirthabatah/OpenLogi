//! OS input-event synthesis for each [`Action`], split out of roadie-core so
...
#[cfg(any(target_os = "linux", target_os = "macos", target_os = "windows"))]
use roadie_core::binding::KeyboardUsage;
use roadie_core::binding::{Action, KeyCombo};
...
/// [`execute`] synthesizes on macOS, so OpenRoadie's own `CGEventTap` can
...
mod tests {
    ...
    #[cfg(any(target_os = "linux", target_os = "macos", target_os = "windows"))]
    use roadie_core::binding::KeyCombo;
