...
/// location.
///
/// Posted at the HID tap location, so OpenRoadie's own event tap sees the
...
/// buttons ≥ 3. Tagged via [`tag_synthetic`] so OpenRoadie's own event tap
...
/// into the event's source user-data so OpenRoadie's own event tap recognises
...
mod tests {
    ...
    use roadie_core::binding::Shortcut;
