...
/// D-Bus proxy for the OpenRoadie GNOME Shell extension. Only the blocking proxy
/// is generated (`gen_async = false`), matching the synchronous poll contract.
#[proxy(
    interface = "org.roadie.Frontmost",
    default_service = "org.roadie.Frontmost",
    default_path = "/org/roadie/Frontmost",
    gen_async = false
)]
trait Frontmost {
...
/// Frontmost backend talking to the OpenRoadie GNOME Shell extension over the
/// session bus.
...
impl GnomeShellSource {
    fn connect() -> Option<Self> {
        ...
        // OpenRoadie extension is installed and exporting the service. An error
        ...
        proxy
            ...
            .map_err(|e| debug!("gnome-shell: OpenRoadie extension not reachable: {e}"))
            .ok()?;
        Some(Self { conn })
    }
}
