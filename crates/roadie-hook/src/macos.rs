...
fn translate(etype: CGEventType, event: &CGEvent) -> Option<MouseEvent> {
    // Skip events OpenRoadie itself synthesised, so a remapped click or inverted
    ...
    if can_be_synthetic
        && event.get_integer_value_field(EventField::EVENT_SOURCE_USER_DATA)
            == roadie_inject::SYNTHETIC_EVENT_USER_DATA
    {
        return None;
    }
    match etype {
...
fn spawn_callback_watchdog(
    ...
    thread::Builder::new()
        .name("roadie-hook-watchdog".into())
        .spawn(move || {
            loop {
...
fn spawn_lifecycle_watchdog(
    ...
    thread::Builder::new()
        .name("roadie-hook-lifecycle-watchdog".into())
        .spawn(move || {
