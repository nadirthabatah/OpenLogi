//! OS-level mouse-event hook for OpenRoadie.
//!
...
//!
//! ```no_run
//! use roadie_hook::{Hook, MouseEvent, EventDisposition};
//!
...
pub use roadie_core::app::ForegroundApp;
pub use roadie_core::binding::ButtonId;
pub use roadie_core::scroll::ScrollDelta;

/// Logitech's USB/Bluetooth vendor id (`0x046D`), widened from
/// [`roadie_core::hid::LOGITECH_VENDOR_ID`] because the hook's identity
/// sources (IOKit, evdev) hand it back as a `u32`.
pub const LOGITECH_VENDOR_ID: u32 = roadie_core::hid::LOGITECH_VENDOR_ID as u32;
...
impl EventDevice {
    ...
    /// Whether this is a Logitech product OpenRoadie may remap buttons for.
    #[must_use]
...
pub enum TapLocation {
    ...
    /// or wedged owner adds latency to every event. This is where OpenRoadie (and
...
impl EventTapInfo {
    ...
    /// with OpenRoadie for the mouse stream, return its product name — used to
...
impl Hook {
    ...
    /// or OpenRoadie's own tap being unexpectedly disabled. Needs no Accessibility
...
/// agent's 1 Hz polling cadence (`roadie_agent_core::watchers::foreground_app`).
#[must_use]
