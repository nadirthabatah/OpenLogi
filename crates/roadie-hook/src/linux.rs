...
/// Prefix carried by every uinput device OpenRoadie creates — the hook's
/// pass-through mice ([`VIRTUAL_DEVICE_NAME`]) and roadie-inject's
/// "OpenRoadie action injector" (which also advertises mouse buttons).
/// Enumeration refuses anything with this prefix so the hook can never grab
/// one of our own virtual devices.
const ROADIE_DEVICE_PREFIX: &str = "OpenRoadie ";

/// Name stamped on every uinput pass-through device.
const VIRTUAL_DEVICE_NAME: &str = "OpenRoadie virtual mouse";
        ...
        let result = (|| -> io::Result<()> {
            for (path, device) in devices {
                ...
                let handle = thread::Builder::new()
                    .name(format!("roadie-hook:{}", path.display()))
                    .spawn(move || {
...
fn find_mouse_devices() -> Vec<(std::path::PathBuf, Device)> {
    evdev::enumerate()
        // Only Logitech mice: the hook exists to remap devices OpenRoadie
...
/// and built-in sticks are never OpenRoadie's target hardware.
///
/// Only Logitech devices are grabbed: OpenRoadie remaps Logitech mice, and an
...
fn is_hookable_mouse(
    ...
    if name.is_some_and(|n| n.starts_with(ROADIE_DEVICE_PREFIX)) {
        return false;
    }
    // OpenRoadie only remaps Logitech hardware — never grab foreign vendors.
...
/// match (`roadie_core::Config::effective_bindings`), so its exact form
            ...
            if session == SessionKind::Wayland && source.name() == "x11" {
                debug!(
                    "frontmost: on Wayland but using the X11/XWayland backend; \
                     native Wayland windows will report None. Install the OpenRoadie \
...
mod tests {
    ...
    fn own_virtual_devices_are_not_hookable() {
        // Both uinput devices OpenRoadie creates carry the prefix; even with
        // fully mouse-like capabilities they must never be grabbed.
        for name in ["OpenRoadie virtual mouse", "OpenRoadie action injector"] {
            let mut caps = Caps::mouse();
            caps.name = Some(name);
    ...
    fn virtual_device_name_carries_exclusion_prefix() {
        assert!(
            VIRTUAL_DEVICE_NAME.starts_with(ROADIE_DEVICE_PREFIX),
