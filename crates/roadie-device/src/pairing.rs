...
//! `BoltReceiver` is closed to extension. So OpenRoadie drives the receiver's
...
// no HID++/backend I/O, so they live in `roadie_core::hid::pairing`;
// re-exported here unchanged so this module's own API surface doesn't churn.
pub use roadie_core::hid::pairing::{Click, PairingError, PasskeyMethod, ReceiverSelector};
