...
// they live in `roadie_core::hid::dpi`; re-exported here unchanged so this
// module's own API surface doesn't churn.
pub use roadie_core::hid::dpi::{Dpi, DpiCapabilities, DpiInfo};

/// Sensor 0 is the only sensor OpenRoadie drives: the UI exposes one DPI value
...
/// its successor, and some mice expose only the latter (`roadie diag
...
/// Used by `roadie diag dpi` and any future Settings → Diagnostics
