...
/// identified before OpenRoadie maps them to a first-class button.
#[derive(Debug, Clone, Copy)]
...
/// `roadie diag features` to confirm which DPI / SmartShift / etc.
...
/// `roadie diag battery`: surfaces exactly what the firmware reports so a
...
/// on many models, a separate radio stack. `roadie diag features` prints
