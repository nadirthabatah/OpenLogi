...
// `roadie_core::hid::error`; re-exported here unchanged so this module's
// own API surface doesn't churn. The conversions below stay here because
// they name `hidpp` and backend types, which `roadie-core` must never
// depend on.
pub use roadie_core::hid::{HidppFeatureErrorKind, HidppOperation, WriteError};
