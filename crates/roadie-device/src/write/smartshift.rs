...
use roadie_core::hid::smartshift::{
...
/// Map the fork's `0x2110` [`WheelMode`] onto OpenRoadie's [`SmartShiftMode`].
/// A future `#[non_exhaustive]` variant maps to [`SmartShiftMode::Ratchet`],
/// the "safe" clicky default OpenRoadie uses elsewhere. (Reserved wire bytes
...
/// Map OpenRoadie's [`SmartShiftMode`] onto the fork's `0x2110` [`WheelMode`] —
