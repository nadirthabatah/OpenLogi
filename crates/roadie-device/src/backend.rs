//! The contract between OpenRoadie's HID++ layer and the HID stack beneath it.
//!
...
//! `roadie-hid` over `async-hid` today, a scripted device tree in tests, and
...
/// The HID stack beneath OpenRoadie's HID++ layer.
///
...
///
/// [`WriteError`](roadie_core::hid::WriteError) is `Serialize` and
...
/// `roadie-core` must never depend on a backend.
impl From<BackendError> for roadie_core::hid::WriteError {
...
impl From<BackendError> for roadie_core::hid::PairingError {
    fn from(error: BackendError) -> Self {
        Self::Hid(error.to_string())
    }
}
