...
// commands_for_light_settings operates purely on roadie_core config/device
// types with no HID++ I/O, so it lives in `roadie_core::hid::light`;
// re-exported here unchanged so this module's own API surface doesn't churn.
pub use roadie_core::hid::light::commands_for_light_settings;
