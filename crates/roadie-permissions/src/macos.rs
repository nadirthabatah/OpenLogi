...
/// Current Camera (AVFoundation) authorization status, via `roadie-camera`,
/// which owns the camera FFI.
#[must_use]
pub fn camera() -> PermissionStatus {
    match roadie_camera::camera_authorization() {
        roadie_camera::CameraAuthorization::Granted => PermissionStatus::Granted,
        roadie_camera::CameraAuthorization::Denied => PermissionStatus::Denied,
        roadie_camera::CameraAuthorization::Undetermined => PermissionStatus::Unknown,
    }
}
...
/// `crates/roadie-desktop/src/platform/AGENTS.md`.
