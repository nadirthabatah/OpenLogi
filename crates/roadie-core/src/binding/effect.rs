...
//! three `roadie-inject` backends don't care about most of that
...
///
/// `roadie_inject`'s per-OS backends match on this instead of on
/// [`Action`] directly.
...
pub enum Effect<'a> {
    ...
    /// `roadie_inject::execute` does open it: that happens in the
...
impl Action {
    ...
    /// `roadie-inject`'s backends dispatch on.
    ///
