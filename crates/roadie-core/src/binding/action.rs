...
/// lives in the `roadie-inject` crate (`roadie_inject::execute`), keeping
...
pub enum Action {
    ...
    ///
    /// Holds the structured chord data so `roadie_inject::execute` can post the
    ...
    /// (`roadie-inject`) runs them in order, awaiting `Delay`s. Power-user
    /// escape hatch — excluded from the default catalog.
