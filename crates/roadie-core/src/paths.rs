...
//! familiar `~/.config/roadie/` rather than macOS's
//! `~/Library/Application Support/`.
//!
//! | kind   | env override        | default                       |
//! |--------|---------------------|-------------------------------|
//! | config | `$XDG_CONFIG_HOME`  | `~/.config/roadie`          |
//! | data   | `$XDG_DATA_HOME`    | `~/.local/share/roadie`     |
//! | state  | `$XDG_STATE_HOME`   | `~/.local/state/roadie`     |
//!
//! On Windows `$HOME` falls back to `%USERPROFILE%`, so paths resolve to
//! `%USERPROFILE%\.config\roadie` etc.
//!
...
//! same layout under an `roadie-dev` app directory.
...
pub const DEV_APP_DIR: &str = "roadie-dev";
...
/// `ROADIE_PROFILE=dev`/`prod`, or (macOS) detected from the bundle the
...
fn detect_dev_profile() -> bool {
    match std::env::var("ROADIE_PROFILE") {
...
/// The raw XDG config home directory (without the `roadie` subdirectory).
///
...
///
/// `$XDG_CONFIG_HOME/roadie`, default `~/.config/roadie`.
/// Local macOS dev builds use `roadie-dev` instead.
...
///
/// `$XDG_DATA_HOME/roadie`, default `~/.local/share/roadie`.
/// Local macOS dev builds use `roadie-dev` instead.
...
///
/// `$XDG_STATE_HOME/roadie`, default `~/.local/state/roadie`.
/// Local macOS dev builds use `roadie-dev` instead.
...
mod tests {
    ...
    #[test]
    fn config_dir_keeps_roadie_under_xdg_config_home() {
        assert!(config_dir().expect("config dir").ends_with("roadie"));
    }

    #[test]
    fn data_dir_keeps_roadie_under_xdg_data_home() {
        assert!(data_dir().expect("data dir").ends_with("roadie"));
    }

    #[test]
    fn runtime_dir_keeps_roadie_suffix() {
        assert!(runtime_dir().expect("runtime dir").ends_with("roadie"));
    }
}
