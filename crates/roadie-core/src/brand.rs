...
//! the `roadie://` deep-link command vocabulary.
//!
...
/// (`org.roadie.agent`, `org.roadie.roadie-dev`), and the value the Linux
...
/// self-identifies OpenRoadie can never disagree. The `.desktop` file carries its
/// own literal copy (it can't reference Rust) — keep the two in sync.
pub const APP_ID: &str = "org.roadie.roadie";
...
pub const AGENT_ID: &str = "org.roadie.agent";

/// The Actions Ring overlay's bundle identifier, the second nested login item.
pub const OVERLAY_ID: &str = "org.roadie.overlay";
...
/// [`AGENT_ID`], although the two look related: `org.roadie.agent` is the
/// frozen label of the legacy hand-written `~/Library/LaunchAgents` plist
/// (see `roadie-agent/src/autostart/macos.rs`), and reusing a legacy label
...
pub const AGENT_SERVICE_LABEL: &str = "org.roadie.agent.service";
...
/// of OpenRoadie's own three processes.
///
/// The frontmost-app reader sees the GUI whenever its window is in front, so
/// without this OpenRoadie would offer itself as a target for a per-app profile.
...
/// `packaging/windows/OpenRoadie.wxs` carries its own literal copy of those names
/// (it can't reference Rust) — keep the two in sync.
#[must_use]
pub fn is_roadie_foreground_id(id: &str) -> bool {
    /// Installed names from `OpenRoadie.wxs`, plus the cargo artifact name a dev
    /// build runs under.
...
/// an `roadie://<name>` URL. macOS delivers it to the running GUI via an
...
impl DeeplinkCommand {
    /// The URL scheme OpenRoadie registers with LaunchServices.
    pub const SCHEME: &str = "roadie";
    ...
    /// Build the `roadie://<name>` URL for this command.
    #[must_use]
    pub fn to_url(self) -> String {
        format!("{}://{}", Self::SCHEME, self.as_name())
    }

    /// Parse a command from its wire name (the part after `roadie://`).
    #[must_use]
    ...
    /// Parse a full `roadie://…` URL. The command lives in the URL's host
    /// component, so any trailing path or query (`roadie://show/`,
    /// `roadie://show?x=1`) is ignored. Returns `None` for a foreign scheme
...
mod tests {
    use super::{
        AGENT_ID, APP_ID, DeeplinkCommand, OVERLAY_ID, dev_id, is_dev_id, is_roadie_foreground_id,
    };
    ...
    fn parse_url_ignores_trailing_path_and_query() {
        assert_eq!(
            DeeplinkCommand::parse_url("roadie://show/"),
            Some(DeeplinkCommand::Show)
        );
        assert_eq!(
            DeeplinkCommand::parse_url("roadie://open-settings?from=tray"),
    ...
    fn parse_url_rejects_foreign_scheme_and_unknown_command() {
        ...
        assert_eq!(DeeplinkCommand::parse_url("roadie://bogus"), None);
        assert_eq!(DeeplinkCommand::parse_url("roadie://"), None);
    }
    ...
    fn the_legacy_dotted_suffix_still_reads_as_dev() {
        ...
        assert!(is_dev_id("org.roadie.agent.dev"));
        assert!(is_dev_id("org.roadie.roadie.dev"));
    }
    ...
    fn matching_ignores_case_but_not_position() {
        assert!(is_dev_id("org.roadie.agent-DEV"));
        assert!(!is_dev_id("org.roadie.dev-agent"));
    }

    #[test]
    fn our_own_processes_are_recognised_in_both_identifier_shapes() {
        for id in [APP_ID, AGENT_ID, OVERLAY_ID] {
            assert!(is_roadie_foreground_id(id), "{id}");
            assert!(is_roadie_foreground_id(&dev_id(id)), "dev {id}");
        }
        // Windows reports a lower-cased executable path; a dev build runs the
        // cargo artifact out of `target/`.
        assert!(is_roadie_foreground_id(
            r"c:\program files\roadie\roadie.exe"
        ));
        assert!(is_roadie_foreground_id(
            r"c:\program files\roadie\roadie-agent.exe"
        ));
        assert!(is_roadie_foreground_id(
            r"c:\src\roadie\target\debug\roadie-desktop.exe"
        ));
    }

    #[test]
    fn a_foreign_app_that_merely_starts_the_same_way_is_not_ours() {
        assert!(!is_roadie_foreground_id("org.roadie.roadie.helper"));
        assert!(!is_roadie_foreground_id(r"c:\apps\openlogic.exe"));
        assert!(!is_roadie_foreground_id(r"c:\apps\roadiec.exe"));
        assert!(!is_roadie_foreground_id("com.apple.Safari"));
        assert!(!is_roadie_foreground_id(""));
    }
}
