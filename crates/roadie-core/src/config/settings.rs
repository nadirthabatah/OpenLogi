...
pub enum AppIcon {
    ...
    /// another.
    ///
    /// Persists as `"roadie"`, and still reads the two spellings that exist in
    /// configs written before the rename — `"openlogi"` from upstream, and the
    /// bare `"roadie"` the docs briefly showed. Variant names are an on-disk
    /// schema contract; a rename without an alias is how a working config file
...
pub enum AssetSourcePreference {
    ...
    #[default]
    Automatic,
    /// Use OpenRoadie's official asset endpoint.
    #[serde(rename = "roadie")]
    OpenRoadie,
...
pub struct AppSettings {
    ...
    /// the app makes exactly one `HEAD /repos/AprilNEA/OpenRoadie/releases/
    ...
    /// Whether OpenRoadie shows a macOS menu-bar (status item) icon — and, on
    ...
    /// `assets.roadie.org` when a device appears. `true` (default) keeps
    ...
    /// Defaults to racing all built-in mirrors; `ROADIE_ASSETS` remains a
    ...
    /// matching a gpui-component theme, e.g. `"OpenRoadie Light"`). `None` uses
    /// the OpenRoadie brand light theme.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub theme_light: Option<String>,
    /// Name of the theme used in dark mode. `None` uses the OpenRoadie brand dark
    /// theme.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Traditional vertical mouse-wheel responsiveness on OpenRoadie's `1..=100`
/// scale.
///
...
/// Thumb-wheel responsiveness on OpenRoadie's `1..=100` scale.
#[nutype(
    const_fn,
    validate(greater_or_equal = SENSITIVITY_MIN, less_or_equal = SENSITIVITY_MAX),
    derive(
...
/// `roadie-ipc/tests/wire_format.rs`).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Lighting {
...
/// Smallest auto-disengage threshold OpenRoadie will store or apply (`8` ≈
...
mod app_icon_tests {
    ...
    /// The value written by upstream OpenLogi before the rename. A config that
    /// worked yesterday must parse today; this is the schema contract the
    /// alias exists to keep.
    #[test]
    fn the_pre_rename_icon_value_still_parses() {
        let held: Holder = toml::from_str("icon = \"openlogi\"").expect("the old value parses");
        assert_eq!(held.icon, AppIcon::OpenRoadie);
    }

    #[test]
    fn the_canonical_value_round_trips() {
        assert_eq!(AppIcon::OpenRoadie.to_string(), "roadie");
        let held: Holder = toml::from_str("icon = \"roadie\"").expect("the new value parses");
        assert_eq!(held.icon, AppIcon::OpenRoadie);
    }
}
