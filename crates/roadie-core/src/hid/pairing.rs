...
//! register writes) lives in `roadie_hid::pairing`.
...
/// the agent maps it to `roadie_ipc::PairingFailure`, which crosses
/// the IPC boundary. The conversion from `roadie_hid::BackendError` lives in
/// `roadie_hid::pairing`, which this crate must never depend on.
