...
//! live in `roadie_hid::write::error`, which this crate must never depend
//! on.
...
/// by `roadie-ipc/tests/wire_format.rs`).
#[derive(Debug, Clone, Error, Serialize, Deserialize)]
pub enum WriteError {
    /// HID transport error serialized as text.
    ///
    /// `roadie_hid::BackendError` isn't `Serialize`, so carry its message as
...
pub enum HidppFeatureErrorKind {
    ...
    Unsupported,
    /// Error code not modeled by OpenRoadie.
    Unrecognized,
}
