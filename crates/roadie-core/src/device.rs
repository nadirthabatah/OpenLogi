...
pub enum DeviceKind {
    /// Mice — the family OpenRoadie's binding/DPI panels primarily target.
    Mouse,
    ...
    Headset,
    /// Logitech webcam (UVC), configured through `roadie-camera`.
    Camera,
...
impl DeviceKind {
    /// Parse the OpenRoadie asset registry's `type` string into a [`DeviceKind`].
    ///
...
/// device against external registries (e.g. the OpenRoadie asset index).
///
...
impl DeviceModelInfo {
    ...
    /// bindings, etc.) and to look up assets in the OpenRoadie asset registry.
    ///
...
pub struct StandaloneDevice {
    ...
    /// Optional model-level identity in the OpenRoadie asset registry.
    ///
...
/// bump (guarded by `roadie-ipc/tests/wire_format.rs`).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
