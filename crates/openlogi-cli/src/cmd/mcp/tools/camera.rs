...
//! what `openlogi camera` already does: UVC controls are a class standard the
...
pub fn list_cameras() -> Result<String, String> {
    let cameras = openlogi_camera::enumerate_all_cameras();
    if cameras.is_empty() {
        return Ok(
...
pub fn read_camera_controls(arguments: &Value) -> Result<String, String> {
    ...
    let state = openlogi_camera::read_camera_state(&id)
